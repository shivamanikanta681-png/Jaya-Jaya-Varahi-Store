...
- **Owner Administration Console**: Password-protected dashboard (Configured securely via `ADMIN_PASSWORD` in your `.env` file) to manage inventory, edit product prices, and review customer orders.
...
### 2. Setup Credentials
Copy `.env.example` to `.env` and fill in your Supabase, Admin Password, and optional Gmail SMTP credentials:
```bash
cp .env.example .env
```
...
├── rag_engine.py           # Multilingual RAG pipeline & catalog retrieval
├── multilingual_kb.py      # Multilingual shopping assistant localization
├── supabase_client.py      # Python Supabase connection manager
├── supabaseClient.js       # Frontend browser Supabase client
├── firebaseClient.js       # Firebase Firestore read client
